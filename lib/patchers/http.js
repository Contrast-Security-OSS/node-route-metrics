...
function patchServer(m, protocol) {
  ...
  shimmer.wrap(proto, 'emit', realEmitter => function(type, req, res) {
    //
    // todo "upgrade" requests? they don't return an http response, it's
    // basically a socket that a stream is written to. do wait for the
    ...
    shimmer.wrap(res, 'end', fn => function() {
      ...
      if (res[metrics]) {
        ...
        // delete the metrics after emitting; express can call end()
        // more than once.
        delete res[metrics];
      }
