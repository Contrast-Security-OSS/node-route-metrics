...
class TimeSeries {
  ...
  constructor(ms, opts) {
    ...
    // now setup cpu and memory monitoring, which always are active. don't allow
    // the averagingInterval to be set; avoid allowing it be too short - there's
    // just no benefit to it.
    this.averagingInterval = defaultOpts.averagingInterval;
    ...
    const intervalHandler = () => {
      ...
      if (delta >= this.averagingInterval) {
        ...
        // get memory usage
        // this should not be called very often as it's expensive. it's also
        ...
        const cpuAndMem = {
          ...
          external: this.memExternalEMA.update(mem.external),
        };
