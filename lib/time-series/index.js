...
class TimeSeries {
   ...
   * - averagingInterval minimum interval for averaging and reporting memory and cpu.
   */
  constructor(ms, opts) {
    this.ms = ms;
    ...
    // now setup cpu and memory monitoring, which always are active. don't make
    // the averaging interval env-var settable to avoid allowing it be too short.
    // there's a cost and no benefit to doing so. if the user did not supply a
    // callback, don't even average.
    this.averagingInterval = this.opts.averagingInterval;
    if (this.tsCallback === defaultOpts.tsCallback) {
      this.averagingInterval = Infinity;
    }
    ...
    // rss and heapTotal are just reported as their current values
    ...
    // don't average more often that averagingInterval.
    this.lastAverage = Date.now();
    ...
    const intervalHandler = () => {
      ...
      const now = Date.now();
      const delta = now - this.lastAverage;
      if (delta >= this.averagingInterval) {
        ...
        // and agent and 2) observe a memory leak. neither of those requires
        // high resolution observations.
        // ignore mem.arrayBuffers as they are included in mem.external.
        ...
        const cpuAndMem = {
          ...
          heapUsedAvg: this.memHeapUsedEMA.update(mem.heapUsed),
          externalAvg: this.memExternalEMA.update(mem.external),
        };
