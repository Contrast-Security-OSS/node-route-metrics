...
const {PerfObserver, monitorEventLoopDelay} = perf_hooks;
...
class Interval {
  constructor(ms, opts) {
    ...
    const intervalHandler = () => {
      if (this.gcCallback) {
        this.gcCallback({count: this.gcCounts, totalTime: this.totalGCTime});
      }
    ...
    const observer = new PerfObserver((list) => {
      ...
      for (const entry of entries) {
        if (entry.entryType === 'gc') {
          ...
          this.gcCounts += 1;
          this.totalGCTime += entry.duration;
        }
      }
    });
...
if (require.main === module) {
  const thing = new Interval(1000, {gc: false});
