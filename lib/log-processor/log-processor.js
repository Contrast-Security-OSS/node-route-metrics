...
class LogProcessor {
   ...
   * the file is opened with the 'w' flag, but this should enable it (with
   * other changes to summarization and reporting).
   */
  async processFile() {
  ...
  summarize() {
    ...
    this.reporter.reportOverall(this.stream, overallInfo);

    //
    // clone each summary and generate an individual report
    //
    for (const summary of this.summaries) {
      const clone = Object.assign({}, summary);
      clone.metrics = [...summary.metrics.entries()];
      clone.patches = [...summary.patches];
      this.individualSummary(clone);
    }
  }

  /**
   * Generate information for a reporter to summarize each individual
   * run summary.
   *
   * @param {object} summary a slightly modified summary
   */
  individualSummary(summary) {
    ...
    const meta = {keyToProperties};
    const options = {template: this.options.template};

    this.reporter.report(this.stream, summary, meta, options);
  }
