...
import expected, {fix} from '../../test/log-files/expected.mjs';
...
desc('log-processor - stdout', function() {
  ...
  for (const file of files) {
    ...
    it(`${file}: read/process with default reporter (csv)`, async function() {
      ...
      const stdout = fix(results.stdout.toString());
    ...
    it(`${file}: csv-deprecated output is the same`, async function() {
      const previous = fix(results.stdout.toString());
    ...
    it(`${file}: read/process with json reporter`, async function() {
      ...
      assert.strictEqual(results.status, 0);
      const stdout = fix(results.stdout.toString()).replaceAll(os.EOL, '\n');
    ...
    it(`${file}: json-deprecated output is the same`, async function() {
      const previous = fix(results.stdout.toString()).replaceAll(os.EOL, '\n');
      const env = Object.assign({}, process.env, {CSI_RM_REPORTER: 'json-deprecated'});
      results = spawnSync('node', ['lib/log-processor/index.mjs', `./test/log-files/${file}`], {env});

      assert.strictEqual(results.status, 0);
      const newResults = fix(results.stdout.toString());
      assert.strictEqual(newResults.length, previous.length);
      assert.strictEqual(fix(results.stdout.toString()), previous);
    });
  }
});
