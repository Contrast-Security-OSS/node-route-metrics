...
function makeHeader(version, config) {
  ...
  header.execArgv = process.execArgv;
  header.node_version = process.version;

  header.os = getOsInfo(errors);
  const pdj = getPackageJson(header.argv[1]);
  if (pdj) {
    header.package_json = pdj[1];
    header.app_dir = pdj[0];
  } else {
    errors.push(`cannot find package.json for application: ${header.argv[1]}`);
    header.package_json = undefined;
    header.app_dir = '.';
  }
