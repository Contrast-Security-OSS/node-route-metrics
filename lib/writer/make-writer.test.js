...
describe('make-writer tests', function() {
  ...
  it('writes objects as timestamped json with a newline', async function() {
    ...
    writer.write({type: 'test'}, obj);
    await asyncFn(stream);
    const file = await fsp.readFile(filename, {encoding: 'utf8'});
    const fileObj = JSON.parse(file);
    const expected = {ts: fileObj.ts, type: 'test', tid: 0, entry: obj};
  ...
  it('writes strings as timestamped json with a newline', async function() {
    ...
    writer.write({type: 'test'}, string);
    await asyncFn(stream);
    const file = await fsp.readFile(filename, {encoding: 'utf8'});
    const fileObj = JSON.parse(file);
    const expected = {ts: fileObj.ts, type: 'test', tid: 0, entry: string};
    expect(fileObj).eql(expected);
  });
  ...
  it('waits as needed', async function() {
    ...
    const extra = JSON.stringify({ts: Date.now(), type: 'test', tid: 0, entry: ' '});
      ...
      stream.on('drain', function() {
        ...
        fsp.readFile(filename, {encoding: 'utf8'})
          .then(text => {
            ...
            expect(objs[0]).property('entry').a('string');
            expect(objs[0].entry).equal(string);
            expect(objs[1]).property('entry').a('string');
            expect(objs[1].entry).equal('x');
            resolve();
          })
          .catch(reject);
      });
  ...
  it('handles stream errors', function(done) {
    ...
    stream.on('finish', function() {
      writer.write({type: 'after-end'}, 'something');
