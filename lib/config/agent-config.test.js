...
describe('agent-config tests', function() {
  beforeEach(function() {
  ...
  it('report invalid values for options', function() {
    process.env[`${prefix}EVENTLOOP`] = 't';
    const {config, errors} = getConfig();
    expect(config).eql(defaultConfig);
    checkErrors(errors, {invalid: [`${prefix}EVENTLOOP must be true or false, not t`]});
  });

  it('does not consider reporter options as unknown to agent', function() {
