...
const csvPath = require.resolve('../log-processor/reporters/csv.cjs');
const jsonPath = require.resolve('../log-processor/reporters/json.cjs');
...
describe('log-processor-config tests', function() {
  ...
  it('returns the right value in errors.invalid when a non-existent template is passed', function() {
    ...
    expect(errors.invalid[0]).contains('CSI_RM_TEMPLATE=xyxxy.x\nTemplate Error: Cannot find module');
  });
});
